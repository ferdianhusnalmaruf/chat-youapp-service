...
export const conversationService = {
  async createConversation(input: CreateConversationInput): Promise<Conversation> {
    const conversation = await conversationRepository.createConversation(input);
  ...
  async lastSeenMessage(conversationId: string, userId: string): Promise<void> {
    await conversationRepository.lastSeenConversation(conversationId, userId);
